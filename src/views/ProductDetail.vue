<template>
  <div class="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-md shadow-md">
    <button
      @click="$router.back()"
      class="mb-4 flex items-center text-red-600 hover:text-red-800 font-semibold text-sm"
    >
    ...
    <div v-else>
      ...
      <div class="border-t mt-5 p-4">
        <div class="flex justify-end gap-4">
          <p class="text-red-600 font-semibold mb-4 p-2">
...
export default {
  ...
  computed: {
    canRedeem() {
      return (
        this.product &&
        parseInt(this.awardPoints) >= parseInt(this.product.pp_point)
      );
    },
  },
  mounted() {
    ...
    async redeemPoints() {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Kurangi poin di localStorage (optional, bisa fetch ulang juga)
          const oldPoints = parseInt(
            localStorage.getItem("award_points") || "0",
            10
          );
          localStorage.setItem(
            "award_points",
            oldPoints - parseInt(this.product.pp_point)
          );

          // Pindah ke halaman utama dan auto buka tab ke Penukaran Saya
          localStorage.setItem("autoTab", "2");
          window.location.href = "/tukarpoin"; // atau path utama kamu
        } else {
